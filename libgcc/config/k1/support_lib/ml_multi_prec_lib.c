...
ml_dd_t ml_split_dd_d_safe(double x) {
    /* Veltkamp split appied to p = 53, C = (1 << 27) + 1 */
    #ifdef __K1__
    //ml_uint32_t ev = __k1_fpu_get_exceptions();
    #endif
    const uid_conv_t C = {.u = 0x41a0000002000000ull};
    double gamma = C.d * x;
    double delta = x - gamma;
...
double ml_subnormalize_d_dd_i(ml_dd_t x, int scale_factor) {
    ...
    ml_uint64_t round_cst = 1 << (delta - 1);
    ...
    */
    ...
    ml_uint64_t sticky = double_to_64b_encoding(x.hi) << (64 - (delta-1));
    ml_uint64_t lo_sign = (double_to_64b_encoding(x.hi) ^ double_to_64b_encoding(x.lo)) >> 63;
    ml_uint64_t parity_bit = double_to_64b_encoding(x.hi) & (1 << delta);

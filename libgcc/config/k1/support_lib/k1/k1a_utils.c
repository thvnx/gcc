...
double k1_subnormalize_d_dd_i(ml_dd_t x, int scale_factor) {
    ...
    ml_uint64_t hi_sign = (double_to_64b_encoding(x.hi) & 0x8000000000000000ull);

    ml_uint64_t x_mant = (double_to_64b_encoding(x.hi) & 0x000fffffffffffffull) | ((ml_uint64_t) 1 << 52);

    enum __k1_fpu_rounding_mode rnd_mode = __k1_fpu_get_rounding_mode();
    ml_uint64_t lo_sign = ((double_to_64b_encoding(x.hi) ^ double_to_64b_encoding(x.lo)) >> 63) != 0;
    ml_uint64_t sticky = ((x_mant << (64 - (delta-1))) != 0);
    ml_uint64_t round_cst = delta <= 53 ? (ml_uint64_t) 1 << (delta - 1) : 0;


    double rounded_value = -1;
    ml_uint64_t round_bit = (x_mant & round_cst) != 0;
        
    if (rnd_mode == _K1_FPU_NEAREST_EVEN) {
        double pre_rounded_value = double_from_64b_encoding(double_to_64b_encoding(x.hi) + round_cst);

        ml_uint64_t parity_bit = x_mant & (1 << delta);
        ...
        if (rnd_mode == _K1_FPU_TOWARDS_ZERO) {
            ...
            ml_uint64_t inf_round_cst = delta <= 52 ? (round_cst << 1) : (round_cst);
            if (x.hi > 0) {
...
double k1_round_signed_overflow_fp64(double sign) {
    enum __k1_fpu_rounding_mode rnd_mode = __k1_fpu_get_rounding_mode();
    ml_uint64_t usign = double_to_64b_encoding(sign) >> 63;
    switch(rnd_mode) {
...
float  k1_round_signed_overflow_fp32(float  sign) {
    enum __k1_fpu_rounding_mode rnd_mode = __k1_fpu_get_rounding_mode();
    ml_uint32_t usign = float_to_32b_encoding(sign) >> 31;
    switch(rnd_mode) {
        case _K1_FPU_NEAREST_EVEN:

...
static void TH_InitTimer(void)
{
   ...
   __builtin_kvx_set(PMC,0);
}

/* Reset and start. */
static void TH_StartTimer(void)
{
   __builtin_kvx_set(PM0,0);
}

/* Stop and return. */
static int TH_StopTimer(void)
{
   return __builtin_kvx_get(PM0);
}
...
profile(unsigned int iter)
{
    ...
    pcr = __builtin_kvx_get(PCR);
    res[__builtin_kvx_get(PCR) + 0 - pcr] = timer1;
    res[__builtin_kvx_get(PCR) + 1 - pcr] = timer2;
    res[__builtin_kvx_get(PCR) + 2 - pcr] = timer3;
}

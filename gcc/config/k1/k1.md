...
;; Be sure to have lo* before lq* so that the pattern matching first
;; try an octuple match.
...
(define_insn "*lo_multiple_cached"
  ...
  "!k1_is_uncached_mem_op(SET_SRC(XVECEXP (operands[0], 0, 1)))"
  "lo %O1 = %3[%2]"
...
;; Be sure to have so* before sq* so that the pattern matching first
;; try an octuple match.
